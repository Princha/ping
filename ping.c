...
{    
    ...
    ms100_fd = fopen("./100ms.log", "w+");
    ms200_fd = fopen("./200ms.log", "w+");
    ms500_fd = fopen("./500ms.log", "w+");
    err_fd = fopen("./error.log", "w+");
    fclose(ms100_fd);
    fclose(ms200_fd);
    fclose(ms500_fd);
    fclose(err_fd);
    ...
    {    
        ...
        {     
            if(errno==EINTR)  /*EINTR表示信号中断*/    
                continue;    
            bail("recvfrom error");  
            err_fd = fopen("./error.log", "a+");
            fprintf(err_fd, "recvfrom error");
            fclose(err_fd); 
            printf("error\n"); 
        }    
...
{    
    ...
    if(rtt >= 500)
    {
        ms500_fd = fopen("./500ms.log", "a+");
        fprintf(ms500_fd, "rtt = %.3f ms\r\n", rtt);
        // fwrite("abcefg\n" , 7 , 1, ms100_fd);
        // printf("rtt > 10ms\n");
        fclose(ms500_fd);
    }
    ...
    {
        ms200_fd = fopen("./200ms.log", "a+");
        fprintf(ms200_fd, "rtt = %.3f ms\r\n", rtt);
        // fwrite("abcefg\n" , 7 , 1, ms100_fd);
        // printf("rtt > 10ms\n");
        fclose(ms200_fd);
    }
    ...
    {
        ms100_fd = fopen("./100ms.log", "a+");
        fprintf(ms100_fd, "rtt = %.3f ms\r\n", rtt);
        // fwrite("abcefg\n" , 7 , 1, ms100_fd);
        // printf("rtt > 10ms\n");
        fclose(ms100_fd);
    }
